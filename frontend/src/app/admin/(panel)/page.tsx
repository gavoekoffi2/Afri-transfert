  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi label="Utilisateurs" value={stats?.users ?? '—'} icon="US" />
        <Kpi label="Transactions" value={stats?.transactions ?? '—'} icon="TX" />
        <Kpi
          label="Volume (réussi)"
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-50 text-[11px] font-black tracking-tight text-brand-700">{icon}</span>
      </div>
